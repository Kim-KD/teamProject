...
public class Admin_mvc_controller {
	...
	public ModelAndView user_list(@RequestParam(defaultValue="1") int pageno, @RequestParam(defaultValue="0") String user_status) {
		return new ModelAndView("admin/user_list").addObject("page",svc.userList(pageno, user_status));
	}
	
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	@GetMapping("/cpn_list")
	public ModelAndView cpn_list(@RequestParam(defaultValue="1") int pageno, @RequestParam(defaultValue="1") String user_status) {
		return new ModelAndView("admin/user_list").addObject("page",svc.userList(pageno, user_status));
	}
